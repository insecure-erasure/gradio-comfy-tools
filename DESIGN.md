...
5. The result URL is available for copying (📋) and chaining: Edit/Upscale/Video
   ...
   bottom-left of their output pane (paste an external URL, or use 🔗 to fill
...
11. **Source image URL field** — Edit/Upscale/Video have a transparent text field (bottom-left of the output pane) for the input image URL; `🔗` fills it with the last generation and it persists per tab. Added to the mockup (was not in the original design, which only had attachment buttons in the prompt bar).
